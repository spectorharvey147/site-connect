...
export default function AccountingSetupView() {
  ...
  return <div className="space-y-4">
    <div className="glass-card p-4"><h2 className="text-lg font-bold">GL &amp; Location Setup</h2><p className="text-sm text-muted-foreground">Map existing employees, expense categories and projects for SAP. Project location takes priority over employee location.</p></div>
    {error && <p role="alert" className="text-destructive">{error}</p>}
    <Input aria-label="Search accounting setup" placeholder="Search name, email or code" value={search} onChange={e => setSearch(e.target.value)} />
    <Tabs defaultValue="users"><TabsList className="h-auto flex-wrap"><TabsTrigger value="users">Employees</TabsTrigger><TabsTrigger value="projects">Projects</TabsTrigger><TabsTrigger value="categories">Expenses</TabsTrigger><TabsTrigger value="locations">Locations</TabsTrigger></TabsList>
      ...
      <TabsContent value="projects" className="space-y-3"><p className="text-sm text-muted-foreground">Enter the exact SAP project code on the project. Reports use this code for all its expenses. If it is blank, the cost-code SAP mapping is used, then the existing cost code.</p>{masters.lists.filter(x => ['project','projectcode'].includes(x.type) && x.active && matches(x.value,x.project_code || '',x.project || '')).map(x => <MasterRow key={x.id} kind={x.type} id={x.id} title={x.type === 'project' ? x.value : `${x.project || 'Common'} / ${x.value} (${x.project_code})`} initial={{ sap_project_code: x.sap_project_code || '', sap_location_code: x.sap_location_code || '' }} masters={masters} onSaved={load} />)}</TabsContent>
      <TabsContent value="categories" className="space-y-3"><h3 className="font-semibold">Expense GL accounts</h3>{masters.groups.map(g => <MasterRow key={g.code} kind="group" id={g.code} title={g.name} initial={{ gl_code: g.gl_code }} masters={masters} onSaved={load} />)}<h3 className="pt-3 font-semibold">Map existing categories</h3>{masters.lists.filter(x => x.type === 'category' && x.active && matches(x.value)).map(x => <MasterRow key={x.id} kind="category" id={x.id} title={x.value} initial={{ sap_expense_group: x.sap_expense_group || '' }} masters={masters} onSaved={load} />)}</TabsContent>
